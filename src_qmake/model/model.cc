...
/**
 * @brief проверяет является ли переданный символ операцией
 *
 * @param ch символ
 *
 * @returns true если символ является знаком операции
 */
...
/**
 * @brief проверяет переданное выражение-строку на правильность
 *
...
int Model::Validator(std::string& str) {
  ...
  if (open_brackets != closed_brackets) error = 1;
  if (str[0] == '.' || str[0] == ',') error = 1;

  return error;
}

/**
 * @brief парсит тригонометрическую функцию
 *
 * @param funct - ссылка на строку,
 * начало которой является тригонометрическое выражение
 *
 * @param i - указатель на символ строки-выражения
 * его передвинем после того как считаем функцию
 *
 * @param type - указатель на тип элемента - поменяем в зависимости о тфункции
 *
...
/**
 * @brief определяет приоритет переданного элемента
 *
 * @param type - переданный элемент
 *
 * @return приоритет от 1 до 4
 */
 ...
 *
 * @param char - символ операции
 *
 * @return тип символа 'LexemeEnum type"
 */
...
/**
 * @brief производит сосответствующую операцию
 * со стэками 'operations' и 'numbers'
 *
 * @param oper - тип операции
 *
 * @param c - результат операции
 */
...
/**
 * @brief работает с двумя стеками 'operations' и 'numbers'
 * в соответствии с польской нотацией.
 * производит одну операцию
 */
...
/**
 * @brief Парсит переданную строку и заполняет стэки
 * 'operations' и 'numbers' в соответствии с польской нотацией
 *
 * @param str - ссылка на выражение-строку
 *
...
/**
 * @brief вычисляет переданное выражение
 *
